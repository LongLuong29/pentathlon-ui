    ...
    return (
      <div className="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">
        ...
        <div>
            ...
            <select className="w-full p-2 border rounded" value={selectedAthlete} onChange={(e) => setSelectedAthlete(e.target.value)}>
          ...
          <div>
            <label className="block font-semibold">Chọn nhóm chỉ số sức khỏe:</label>
            <select className="w-full p-2 border rounded" value={selectedMetricGroup} onChange={(e) => setSelectedMetricGroup(e.target.value)}>
        ...
        <div className="mb-4">
          ...
          <select className="w-full p-2 border rounded" value={selectedHealthMetric} onChange={(e) => setSelectedHealthMetric(e.target.value)}>
            <option value="">-- Chọn --</option>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input className="w-full p-2 border rounded" type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} />
          </div>
          <div>
            <label className="block font-semibold">Đến ngày:</label>
            <input className="w-full p-2 border rounded" type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} />
          </div>
        </div>

        <div className="mt-4">
          <h3 className="text-xl font-bold mb-2 text-center">Biểu đồ sức khỏe</h3>
          <Line className="bg-gray-100 p-4 rounded shadow" data={chartData} options={{
            scales: {
              x: {
